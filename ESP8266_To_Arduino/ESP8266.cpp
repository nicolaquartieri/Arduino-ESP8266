...
bool ESP8266::send(uint8_t *buffer, uint32_t len)
{
...
void ESP8266::serial_rx_empty(void) 
{
...
//    data = recvString("OK", "ERROR", "ALREADY CONNECT", 10000);
    data = m_puart->readString();
    Serial.println("DEBUG Connect: " + data);
    if (data.indexOf("OK") != -1 || data.indexOf("ALREA") != -1 || data.indexOf("CONNEC") != -1 ) {
        return true;
    }
    return false;
}
...
bool ESP8266::sATCIPSENDSingle(uint8_t *buffer, uint32_t len)
{
    ...
    m_puart->println(len);
//    data = m_puart->readString();
//    Serial.println("DEBUG sATCIPSENDSingle: " + data);
//    if (data.indexOf('>') != -1) {
//    if (recvFind(">", 5000)) {
        rx_empty();
...
bool ESP8266::eATCIPCLOSESingle(void)
{
    ...
    data = m_puart->readString();
    Serial.println("DEBUG Close: " + data);
    if (data.indexOf("OK") != -1) {
        return true;
    }
    return false;
